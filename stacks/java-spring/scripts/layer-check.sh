#!/usr/bin/env bash
# layer-check.sh — Structural dependency test for Java + Spring Boot
#
# Enforces: DTO → Config → Repository → Service → Controller
...
LANG = 'Java'
EXT = 'java'
RULES = json.loads('[{"layer": "DTO", "source_aliases": ["dto"], "forbidden": ["config", "repository", "service", "controller"]}, {"layer": "Config", "source_aliases": ["config"], "forbidden": ["repository", "service", "controller"]}, {"layer": "Repository", "source_aliases": ["repository"], "forbidden": ["service", "controller"]}, {"layer": "Service", "source_aliases": ["service"], "forbidden": ["controller"]}]')
