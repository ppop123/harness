# CLAUDE.md — Claude 工作指南（C# + ASP.NET Core）
...
- 按层级顺序实现：DTOs → Config → Repositories → Services → Controllers  <!-- harness-init 适配：替换为项目实际分层 -->
- 每完成一个子任务，汇报进度再继续
...
### 完成标准
- [ ] `dotnet build --warnaserror && dotnet test` 全部通过  <!-- harness-init 适配：替换为项目实际命令 -->
...
```
❌ 不要修改与任务无关的文件
❌ 不要使用 public object GetUser(dynamic id) => ...
❌ 不要安装新依赖（先告知我，等确认）
❌ 不要删除注释或测试（除非明确要求）
```
