...
```bash
# 安装依赖
NuGet / dotnet CLI install  # 根据项目实际调整
...
3. 写代码（按层级顺序）
4. 运行检查：dotnet build --warnaserror && dotnet test
5. 提交 PR
```
...
| 命令 | 说明 |
|------|------|
| `dotnet run --project src/ProjectName.Api` | 启动开发 |
| `dotnet format --verify-no-changes` | 代码检查 |
| `dotnet test` | 运行测试 |
| `dotnet build --warnaserror && dotnet test` | 完整检查（提交前跑） |
