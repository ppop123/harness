...
3. 写代码（按层级顺序）
4. 运行检查：npm run lint && npm run test
5. 提交 PR
```
...
| 命令 | 说明 |
|------|------|
| `npx tsx watch src/index.ts` | 启动开发 |
| `eslint src/ && tsc --noEmit` | 代码检查 |
| `npx vitest run` | 运行测试 |
| `npm run lint && npm run test` | 完整检查（提交前跑） |
