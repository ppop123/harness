# CLAUDE.md — Claude 工作指南（TypeScript + Node.js API）
...
- 按层级顺序实现：Types → Config → Utils → Repositories → Services → Controllers → Routes  <!-- harness-init 适配：替换为项目实际分层 -->
- 每完成一个子任务，汇报进度再继续
...
### 完成标准
- [ ] `npm run lint && npm run test` 全部通过  <!-- harness-init 适配：替换为项目实际命令 -->
