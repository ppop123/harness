...
1. **依赖方向单向流动** — `Domain → Repository → Service → Handler → Router`，禁止反向  <!-- harness-init 适配：替换为项目实际分层 -->
2. **边界必须验证** — 外部数据用 go-playground/validator 验证，不猜测结构
3. **不重复** — 复用 utils/lib，超过 2 处重复必须提取
4. **类型安全** — 严格使用 Go 编译器（静态类型）
5. **错误必须处理** — 不允许静默吞掉 error
6. **配置集中** — 用 envconfig / viper 管理环境变量，禁止硬编码
