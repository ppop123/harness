...
| # | 原则 | 检查方式 |
|---|------|---------|
| G1 | 类型安全 | TypeScript strict |
| G2 | 边界验证：外部数据必须校验 | zod |
| G3 | 不重复（DRY）：>2 处重复必须提取 | 代码审计 |
| G4 | 错误必须处理：不允许静默吞掉 | ESLint + @typescript-eslint |
| G5 | 禁止硬编码配置 | expo-constants + zod |
| G6 | 函数单一职责：上限 50 行 | ESLint + @typescript-eslint |
| G7 | 关键路径必须有测试 | Jest + React Native Testing Library |
| G8 | 依赖层级不可违反 | 结构测试 |
...
```
❌ 禁止：const data: any = await fetch(url).then(r => r.json())

✅ 正确：const data = UserSchema.parse(await apiClient.getUser(id))
```
...
所有配置值（API keys、URL、端口等）必须通过 expo-constants + zod 管理。
