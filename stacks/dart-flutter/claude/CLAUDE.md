# CLAUDE.md — Claude 工作指南（Dart + Flutter）
...
- 按层级顺序实现：Models → Data Sources → Repositories → Providers/Blocs → UI  <!-- harness-init 适配：替换为项目实际分层 -->
- 每完成一个子任务，汇报进度再继续
...
### 完成标准
- [ ] `dart analyze && flutter test` 全部通过  <!-- harness-init 适配：替换为项目实际命令 -->
...
```
❌ 不要修改与任务无关的文件
❌ 不要使用 dynamic data = jsonDecode(response.body);
❌ 不要安装新依赖（先告知我，等确认）
❌ 不要删除注释或测试（除非明确要求）
```
