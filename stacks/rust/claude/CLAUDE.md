...
- 按层级顺序实现：Models → Config → Repository → Service → Handler → Router  <!-- harness-init 适配：替换为项目实际分层 -->
- 每完成一个子任务，汇报进度再继续
...
### 完成标准
- [ ] `cargo clippy -- -D warnings && cargo test` 全部通过  <!-- harness-init 适配：替换为项目实际命令 -->
