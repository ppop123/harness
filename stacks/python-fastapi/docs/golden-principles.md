...
|---|------|---------|
| G1 | 类型安全 | mypy --strict |
| G2 | 边界验证：外部数据必须校验 | Pydantic v2 |
| G3 | 不重复（DRY）：>2 处重复必须提取 | 代码审计 |
| G4 | 错误必须处理：不允许静默吞掉 | Ruff |
| G5 | 禁止硬编码配置 | pydantic-settings |
...
```
❌ 禁止：def process_user(user):
    return user['name']

✅ 正确：def process_user(user: User) -> str:
    return user.name
```
...
所有配置值（API keys、URL、端口等）必须通过 pydantic-settings 管理。
